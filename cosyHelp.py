...
def COSYSingleRun(magnets, i = 0, FP = 'FP1'):
	...
	#	'M5 0.250 Q1*SC*' + str(magnets[i][0]) + ' Q1H*SC*' + str(magnets[i][0]) + ' 0 -0.00318*Q1*SC*' + \
	#		str(magnets[i][0]) + ' 0 0.055;        {Q1+Hex}\n')
	replace_line(cosyfileTemp, cosyfileTemp, 263, \
		'MQ 0.2979 Q2*SC*' + str(magnets[i][0]) + ' 0.068;                   			{Q2}\n')
	replace_line(cosyfileTemp, cosyfileTemp, 325, \
		'MQ 0.3499 Q3*SC*' + str(magnets[i][1]) + ' 0.11;                   			{Q3}\n')
	if FP == 'FP1':
		replace_line(cosyfileTemp, cosyfileTemp, 361, \
			'WMI; \n')
		replace_line(cosyfileTemp, cosyfileTemp, 483, \
			'{WMI;} \n')
	elif FP == 'FP2':
		replace_line(cosyfileTemp, cosyfileTemp, 361, \
			'{WMI;} \n')
	...
	mv_cmd = 'mv "%s" "%s"' % (cosyfileTemp, str(i) + '/' + cosyfileTemp)
	subprocess.call(mv_cmd, shell=True)
	temp = open(str(i) + '/' + 'temp.txt', "w") # temporary file for writing COSY terminal output
	subprocess.call([pathToCOSY, cosyfileTemp], stdout=temp, cwd=str(i) + '/')
	temp.close()
...
def COSYParallelRun(magnets, FP):
	...
	for i in range(len(magnets)):
		pool.apply_async( COSYSingleRun, [magnets, i, FP] )
	pool.close()
	pool.join()
